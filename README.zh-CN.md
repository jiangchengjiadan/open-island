...
- **多会话状态，全局可见**：把本地 Claude Code 和 Codex 会话汇聚到一个刘海面板里，清楚区分运行中、等待中、已完成和异常状态。
- **桌面级权限审批**：支持的权限请求会直接出现在面板中，你可以在当前 IDE 视线内完成批准或拒绝，不必切回终端。
- **一键回到现场**：点击会话卡片即可跳回对应的 Terminal、iTerm 或支持的 IDE 路由，减少多项目并行时的上下文迷失。
...
项目结构刻意保持轻量和可扩展：Agent hook、bridge 事件和 SwiftUI/AppKit 面板彼此分离，后续可以继续接入更多本地 Agent，而不需要改变产品核心形态。
